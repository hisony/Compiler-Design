...
bool isValidIdentifier(string variable, const set<string> &keywords)
{
    ...
    {
        cout << "Variable name can't be same as keyword name\n";
        return false;
    }
    ...
    for (int i = 1; i < variable.size(); ++i)
    {
              ...
              (variable[i] >= '0' && variable[i] <= '9')))
        {
            if (variable[i] == ' ')
            {
                cout << "Space should not be included in the variable name.\n";
                return false;
            }

            cout << "'" << variable[i] << "' should not be included in the variable name.\n";
            return false;
        }
    }
...
int main()
{
    ...
    cout << isValidIdentifier(var, keywords);
